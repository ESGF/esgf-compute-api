...
class WPSClient(object):
    ...
    def parse_data_inputs(data_inputs):
        ...
        kwargs = dict(
            (x.split("=")[0], json.loads(x.split("=")[1]))
            for x in match.group(1).split(";")
        )

        variables = [
            Variable.from_dict(x) for x in kwargs.get("variable", [])
        ]

        domains = [Domain.from_dict(x) for x in kwargs.get("domain", [])]

        operation = [
            Process.from_dict(x) for x in kwargs.get("operation", [])
        ]
    ...
    def _build_process_collection(self):
        ...
        for x in processes:
            ...
            if len(id_parts) == 1:
                ...
                raise CWTError(
                    "Could not group process {!s}.", process.identifier
                )
        ...
        for x, y in groups.items():
            ...
            doc = (
                "Collection of processes for {module!r} module.\n"
                "\n"
                "{process_list!s}"
            )
    ...
    def process_by_name(self, identifier, version=None):
        ...
        if process is None:
            raise CWTError(
                "No matching process {!r} version {!r}", identifier, version
            )
    ...
    def _repr_html_(self):
        ...
        for x in self._client.processes:
            short_abstract = (
                x.abstract[:max_length]
                + "...<b>see process for full abstract</b>"
                if len(x.abstract) > max_length
                else x.abstract
            )
            ...
            r = (
                ...
                "</tr>"
            ).format(**x.__dict__, short_abstract=short_abstract)

            rows.append(r)

        table = "<div><table>{}{}</table></div>".format(
            headers, "".join(rows)
        )
    ...
    def prepare_data_inputs(self, process, inputs, domain, **kwargs):
        ...
        """
        data_inputs = utilities.prepare_data_inputs(
    ...
    def _patch_authentication(self, headers, params):
        ...
        if self.auth is not None and isinstance(
    ...
    def _execute_post(self, process, data_inputs, headers):
        ...
        process.context = self._client.execute(
    ...
    def _execute_get(self, process, data_inputs, headers, params):
        data_inputs = ";".join(
            ["{0!s}={1!s}".format(x, y) for x, y in data_inputs.items()]
        )
        ...
        response = requests.get(
            self.url, params=params, headers=headers, **extras
        )
    ...
    def execute(self, process, inputs=None, domain=None, **kwargs):
        ...
        logger.info(
            f"Executing {process.identifier} with {len(process.inputs)}"
        )

        if inputs is not None or domain is not None or len(kwargs) > 0:
            warnings.warn(
                'Use of "inputs", "domain" and setting parameters is deprecated.'
                "Set theses values on their respective process objects.",
                DeprecationWarning,
            )
        ...
        try:
            if isinstance(process, Process):
                if method == "post":
                    ...
                    process = self._execute_post(
                        process, data_inputs, headers
                    )
                elif method == "get":
                    ...
                    process = self._execute_get(
                        process, data_inputs, headers, params
                    )
                else:
